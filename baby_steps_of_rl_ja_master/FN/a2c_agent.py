...
class ActorCriticAgent(FNAgent):
    ...
    def load(cls, env, model_path):
        ...
        agent = cls(actions)
        agent.model = tf.keras.models.load_model(model_path, custom_objects={
    ...
    def make_model(self, feature_shape):
        ...
        actor_layer = tf.keras.layers.Dense(len(self.actions),
                                     kernel_initializer=normal)
        action_evals = actor_layer(model.output)
        actions = SampleLayer()(action_evals)

        critic_layer = tf.keras.layers.Dense(1, kernel_initializer=normal)
    ...
    def set_updater(self, optimizer,
        ...
        self._updater = tf.keras.backend.function(
...
class ActorCriticAgentTest(ActorCriticAgent):
    ...
    def make_model(self, feature_shape):
        normal = tf.keras.initializers.glorot_normal()
        model = tf.keras.Sequential()
        model.add(tf.keras.layers.Dense(10, input_shape=feature_shape,
                                 kernel_initializer=normal, activation="relu"))
