...
import sys
sys.path.append('/Users/ta2bonn/Desktop/datamix/ゼミ/2021年度/強化学習/python/baby_steps_of_rl_ja_master')
from FN.fn_framework import FNAgent, Trainer, Observer

import sys
sys.path.append('/Users/ta2bonn/Desktop/datamix/ゼミ/2021年度/強化学習/python')
import gym_ple_03
...
class DeepQNetworkAgent(FNAgent):
    ...
    def make_model(self, feature_shape):
        normal = tf.keras.initializers.glorot_normal()
        ...
        model.add(tf.keras.layers.Dense(256, kernel_initializer=normal,
                                 activation="relu"))
        model.add(tf.keras.layers.Dense(len(self.actions),
                                 kernel_initializer=normal))
        self.model = model
        self._teacher_model = tf.keras.models.clone_model(self.model)
...
class DeepQNetworkAgentTest(DeepQNetworkAgent):
    ...
    def make_model(self, feature_shape):
        normal = tf.keras.initializers.glorot_normal()
        model = tf.keras.Sequential()
        model.add(tf.keras.layers.Dense(64, input_shape=feature_shape,
                                 kernel_initializer=normal, activation="relu"))
        model.add(tf.keras.layers.Dense(len(self.actions), kernel_initializer=normal,
                                 activation="relu"))
        self.model = model
        self._teacher_model = tf.keras.models.clone_model(self.model)
...
class DeepQNetworkTrainer(Trainer):
    ...
    def begin_train(self, episode, agent):
        optimizer = tf.keras.optimizers.Adam(lr=self.learning_rate, clipvalue=1.0)
